...
const PatientDashboard = () => {
  ...
  // UPDATE: Add state for goals (with defaults)
  ...
  const fetchDashboard = async () => {
    try {
      ...
      //  UPDATE: Update goals from the backend response
      if (res.data.goals) {
  ...
  // UPDATE: Use the dynamic 'goals' state instead of hardcoded numbers
    ...
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      <aside className="w-64 bg-gray-800 text-white flex flex-col hidden md:flex">
        ...
        <nav className="flex-1 mt-6 space-y-2">
          <Link
            to="/dashboard"
            className="block px-6 py-3 bg-gray-700 border-r-4 border-white"
          >
            Dashboard
          </Link>
          <Link
            to="/profile"
            className="block px-6 py-3 text-gray-400 hover:bg-gray-700 hover:text-white"
          >
            My Profile
          </Link>
          <Link
      ...
      <main className="flex-1 p-4 md:p-8">
        <header className="mb-8 flex justify-between items-center">
          ...
          <button onClick={logout} className="md:hidden text-red-500 font-bold">
            Logout
          </button>
        </header>
          ...
          <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-3">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 relative overflow-hidden">
              ...
              <div className="mb-4 flex items-baseline">
                ...
                {/* 🟢 GOAL MET BADGE */}
                {stats.steps >= stepsGoal && (
                  <span className="ml-2 px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-xs font-bold border border-green-200">
                    Goal Met! 🏆
                  </span>
                )}
              </div>
              ...
              <button
                ...
                className="mt-3 w-full py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded border border-gray-300 transition"
              >
                + Add 500 Steps
              </button>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between">
              ...
              <button
                ...
                className="w-full py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded border border-blue-200 transition"
              >
                + Drink Glass
              </button>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between">
              ...
              <div className="mb-2 flex items-baseline">
                ...
                <span className="text-gray-500 text-sm ml-1"> hrs</span>

                {/* 🟢 GOAL MET BADGE */}
                {stats.sleep >= (goals.sleep || 8) && (
                  <span className="ml-2 px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold border border-indigo-200">
                    Rested 🌙
                  </span>
                )}
              </div>
                ...
                {[...Array(8)].map((_, i) => (
                  <div
                    ...
                    className={`h-4 flex-1 rounded-full ${
                      i < stats.sleep ? "bg-indigo-400" : "bg-gray-200"
                    }`}
                  ></div>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded border border-indigo-200 transition"
              >
                ✎ Update Hours
              </button>
            </div>
        ...
        <section className="mb-8 border-t border-gray-300 pt-6">
          <h2 className="text-sm font-bold text-gray-600 uppercase mb-2">
            Reminders
          </h2>
